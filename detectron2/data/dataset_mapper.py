...
class DatasetMapper:
    ...
    def __call__(self, dataset_dict):
        ...
        if "data_dt_file_name" in dataset_dict:
            image_trgt = utils.read_image(dataset_dict["data_dt_file_name"], format=self.image_format)
...
class DatasetMapperTwoCropSeparate(DatasetMapper):
    ...
    def __call__(self, dataset_dict):
        ...
        if "data_dt_file_name" in dataset_dict:
            image_src2trgt = utils.read_image(dataset_dict["data_dt_file_name"], format=self.img_format)
        ...
        if image_src2trgt is not None:
            # aug_input_src2trgt = T.AugInput(image_src2trgt, sem_seg=sem_seg_gt)
            image_src2trgt = transforms.apply_image(image_src2trgt)
        ...
        if image_src2trgt is not None:
            dataset_dict_style = copy.deepcopy(dataset_dict)
            dataset_dict_style["image"] = torch.as_tensor(np.ascontiguousarray(image_src2trgt.transpose(2, 0, 1)))
        ...
        if image_src2trgt is not None:
            assert dataset_dict["image"].size(1) == dataset_dict_style["image"].size(1)
            assert dataset_dict["image"].size(2) == dataset_dict_style["image"].size(2)
            return dataset_dict, dataset_dict_key, dataset_dict_style
        else:
