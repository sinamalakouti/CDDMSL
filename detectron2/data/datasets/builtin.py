...
from .cityscapes import load_cityscapes_instances, load_cityscapes_semantic, load_cityscapes_DG_instances
from .cityscapes_panoptic import register_all_cityscapes_panoptic
from .coco import load_sem_seg, register_coco_instances
from .coco_panoptic import register_coco_panoptic, register_coco_panoptic_separated
from .lvis import get_lvis_instances_meta, register_lvis_instances
from .pascal_voc import register_pascal_voc, register_pascal_DG
...
_RAW_CITYSCAPES_DG_SPLITS = {
    "cityscapes_DG_train": ("cityscapes/leftImg8bit/train/","cityscapes/leftImg8bit_foggy/train/", "cityscapes/gtFine/train/"),
    "cityscapes_DG_val": ("cityscapes/leftImg8bit/val/", "cityscapes/leftImg8bit_foggy/val/", "cityscapes/gtFine/val/"),
    "cityscapes_DG_test": ("cityscapes/leftImg8bit/test/","cityscapes/leftImg8bit_foggy/test/", "cityscapes/gtFine/test/"),
    "cityscapes_val": ("cityscapes/leftImg8bit/val/", None, "cityscapes/gtFine/val/"),
    "cityscapes_foggy_val": ("cityscapes/leftImg8bit_foggy/val/", None, "cityscapes/gtFine/val/"),
}
...
def register_all_cityscapes_DG(root):
    for key, (image_dir, image_dt_dir, gt_dir) in _RAW_CITYSCAPES_DG_SPLITS.items():
        meta = _get_builtin_metadata("cityscapes")
        image_dir = os.path.join(root, image_dir)
        gt_dir = os.path.join(root, gt_dir)

        if image_dt_dir is not None:
            image_dt_dir = os.path.join(root, image_dt_dir)


            inst_key = key
            DatasetCatalog.register(
                inst_key,
                lambda x=image_dir, x_dt=image_dt_dir, y=gt_dir: load_cityscapes_DG_instances(
                    x,x_dt, y, from_json=False, to_polygons=False
                ),
            )
            MetadataCatalog.get(inst_key).set(
                image_dir=image_dir, image_dt=image_dt_dir,  gt_dir=gt_dir, evaluator_type="coco", **meta
            )
            ...
            DatasetCatalog.register(
                inst_key,
                lambda x=image_dir, x_dt=None, y=gt_dir: load_cityscapes_DG_instances(
                    x, x_dt, y, from_json=False, to_polygons=False
                ),
            )
            MetadataCatalog.get(inst_key).set(
...
def register_all_pascal_DG(root):
    SPLITS = [
        ("voc_clipart_2007_trainval", "VOC2007", "trainval", "dt_clipart"),
        ("voc_clipart_2007_train", "VOC2007", "train", "dt_clipart"),
        ("voc_clipart_2012_trainval", "VOC2012", "trainval", "dt_clipart"),
        ("voc_clipart_2012_train", "VOC2012", "train", "dt_clipart"),
        ("voc_watercolor_2007_trainval", "VOC2007", "trainval", "dt_watercolor"),
        ("voc_watercolor_2007_train", "VOC2007", "train", "dt_watercolor"),
        ("voc_watercolor_2012_trainval", "VOC2012", "trainval", "dt_watercolor"),
        ("voc_watercolor_2012_train", "VOC2012", "train", "dt_watercolor"),
        ("voc_comic_2007_trainval", "VOC2007", "trainval", "dt_comic"),
        ("voc_comic_2007_train", "VOC2007", "train", "dt_comic"),
        ("voc_comic_2012_trainval", "VOC2012", "trainval", "dt_comic"),
        ("voc_comic_2012_train", "VOC2012", "train", "dt_comic"),

    ]
    for name, dirname, split, dt_name in SPLITS:
        year = 2007 if "2007" in name else 2012
        register_pascal_DG(name, os.path.join(root, dirname), split, year, dt_data=dt_name)
        MetadataCatalog.get(name).evaluator_type = "pascal_voc"
...
def register_all_clipart(root):
    ...
    SPLITS = [
        ...
        ("dt_Clipart_2007_trainval", "dt_clipart/VOC2007", "trainval"),
        ("dt_Clipart_2012_trainval", "dt_clipart/VOC2012", "trainval"),
    ]
    for name, dirname, split in SPLITS:
        year = 2012
        register_pascal_voc(name, os.path.join(root, dirname), split, year)
...
def register_all_comic(root):
    ...
    SPLITS = [
        ...
        # ("Clipart_dt_train", "dt_clipart", "train")
    ]
    for name, dirname, split in SPLITS:
        year = 2012
        # register_pascal_voc(name, os.path.join(root, dirname), split, year, class_names=["person", "dog","bicycle", "bird", "car", "cat"])
...
if __name__.endswith(".builtin"):
    ...
    register_all_pascal_DG(_root)
    register_all_cityscapes_DG(_root)
